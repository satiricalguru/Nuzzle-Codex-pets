Create Codex pet row `running-right` for `aiko`: exactly 8 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing Aiko identity, proportions, palette, markings, face, materials, props, and pixel-art/chibi style from the supplied 8x9 atlas. This is an upgrade: do not redesign the standard rows.. Preserve silhouette, face, palette, material, proportions, markings, and props.
