Create one horizontal animation strip for Codex pet `conan`, state `running-right`.
...
Identity: same pet in every frame: Preserve the existing Conan identity, proportions, palette, markings, face, materials, props, and pixel-art/chibi style from the supplied 8x9 atlas. This is an upgrade: do not redesign the standard rows.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
