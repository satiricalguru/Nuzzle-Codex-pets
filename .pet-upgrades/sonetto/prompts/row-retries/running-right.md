Create Codex pet row `running-right` for `sonetto`: exactly 8 full-body frames in one horizontal strip on flat pure blue #0000FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing Sonetto identity, proportions, palette, markings, face, materials, props, and pixel-art/chibi style from the supplied 8x9 atlas. This is an upgrade: do not redesign the standard rows.. Preserve silhouette, face, palette, material, proportions, markings, and props.
