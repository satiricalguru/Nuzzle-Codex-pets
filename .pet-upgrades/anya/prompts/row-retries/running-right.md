Create Codex pet row `running-right` for `anya`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve Anya's existing pink-haired chibi girl identity, black dress with gold trim, black hair ornaments, green eyes, and pixel-art style from the supplied atlas.. Preserve silhouette, face, palette, material, proportions, markings, and props.
